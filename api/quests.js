// /api/quests.js — Quests API using Upstash (path-style REST)
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (body && body.title) {
        ...
        if (i>=0) {
          ...
          if (!wasDone && body.done) {
            // award XP (+10) and update daily streak
            const today = new Date().toISOString().slice(0,10);
            const y = new Date(); y.setDate(y.getDate()-1);
            const yesterday = y.toISOString().slice(0,10);
            if (stats.lastDoneDate === yesterday) stats.streak = (stats.streak||0) + 1;
            else if (stats.lastDoneDate !== today) stats.streak = 1;
      ...
      const [saveQ, saveS] = await Promise.all([
        fetch(`${BASE}/set/${encodeURIComponent(keyQuests)}/${encodeURIComponent(JSON.stringify(quests))}`, {
          method:'POST', headers:{ Authorization:`Bearer ${TOKEN}` }
        }).then(r=>r.json()),
        fetch(`${BASE}/set/${encodeURIComponent(keyStats)}/${encodeURIComponent(JSON.stringify(stats))}`, {
          method:'POST', headers:{ Authorization:`Bearer ${TOKEN}` }
        }).then(r=>r.json())
      ]);
